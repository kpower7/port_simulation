...
def circular_kde(data, x, bw=0.1, period=7):
    """
    Compute KDE with circular boundary conditions
    """
    # Wrap the data and evaluation points
    wrapped_data = np.concatenate([data - period, data, data + period])
    kde = gaussian_kde(wrapped_data, bw_method=bw)
    
    # Evaluate KDE and combine wrapped components
    density = kde(x)
...
def evaluate_models(data, category, subplot_idx=1):
    ...
    # Fit regular KDE with fixed bandwidth
    kde = gaussian_kde(X_train, bw_method=0.1)
    
    # Fixed bandwidth for circular KDE
    ...
    circular_density = circular_kde(X_train, x_plot, bw=bw, period=period)
    normal_density = norm.pdf(x_plot, mu, std)
    fourier_density = fourier_series(x_plot_scaled, *fourier_params)
    
    # Normalize densities
    circular_density = circular_density / np.trapz(circular_density, x_plot)
    ...
    kde_test = kde(x_test)
    circular_test = circular_kde(X_train, x_test, bw=bw, period=period)
    ...
    print(f"Circular KDE - MAE: {circular_mae:.4f}, RMSE: {circular_rmse:.4f}")
...
print(f"Containers with both dates: {len(both_dates):,}")
...
plt.tight_layout()
plt.savefig('circular_comparison.png')
print("\nSaved visualization to circular_comparison.png")
...
for category in categories:
    ...
    circular_imp = ((r['normal_mae'] - r['circular_mae']) / r['normal_mae'] * 100)
    fourier_imp = ((r['normal_mae'] - r['fourier_mae']) / r['normal_mae'] * 100)
    
    print(f"| {category:<13} | KDE     | {r['kde_mae']:.4f} | {r['kde_rmse']:.4f} | {kde_imp:>19.1f}% |")
    print(f"| {category:<13} | Circular| {r['circular_mae']:.4f} | {r['circular_rmse']:.4f} | {circular_imp:>19.1f}% |")
