...
class Container(sim.Component):
    def x(self, t):
        if isinstance(self.equipment, RTG):
            ...
            return self.x_now(t) if callable(self.x_now) else self.x_now
    ...
    def y(self, t):
        if isinstance(self.equipment, RTG):
            ...
            return self.y_now(t) if callable(self.y_now) else self.y_now
    ...
    def z(self, t):
        if isinstance(self.equipment, RTG):
            ...
            return self.z_now(t) if callable(self.z_now) else self.z_now
    ...
    def setup(self, length=40, color="lime", edge_color="", equipment=None, x_now=0, y_now=0, z_now=0, angle=0, slot=None, row=None, height=None):
        ...
        self.row = row
        self.height = height
        self.ao = AnimateContainer(
            length=length,
            x=self.x,
            y=self.y,
            z=self.z,
            z_ref=-1,
            z_angle=self.angle,  # Changed from self.z_angle to self.angle
            color=color,
...
class AnimateContainer(sim.Animate3dBox):
    def __init__(self, length, x=0, y=0, z=0, z_angle=0, z_ref=1, color="red", edge_color="white"):
        # Convert color names to fixed RGB tuples
        fixed_color = env.colorspec_to_tuple(color)
        fixed_edge_color = env.colorspec_to_tuple(edge_color)
        ...
        super().__init__(
            ...
            y_ref=0,
            z_ref=z_ref,
            color=fixed_color,           # Use fixed RGB tuple
            edge_color=fixed_edge_color, # Use fixed RGB tuple
            shaded=True,
        )
...
class RTG(sim.Component):
    def setup(self, id, block):
        ...
        if do_animate:
            ...
            self.ao_trolley = AnimateTrolley(x=x, y=y, color="red") 
...
def add_tracks():
    # Track positions
    x_start, x_end = 100, 700  # Start and end of the tracks
    center_y = 55             # Center of the tracks
    track_spacing = CONTAINER_WIDTH + 1 # Distance between the tracks
    y_track_1 = center_y - track_spacing / 2  # First track
    y_track_2 = center_y + track_spacing / 2  # Second track

    # First Track (Top line)
    sim.AnimateLine(
        spec=(x_start, y_track_1, x_end, y_track_1),
        linewidth=2,
        linecolor="gray",
        screen_coordinates=False,
    )
    sim.Animate3dLine(
        x0=x_start, y0=y_track_1, z0=0,
        x1=x_end, y1=y_track_1, z1=0,
        color="gray",
    )

    # Second Track (Bottom line)
    sim.AnimateLine(
        spec=(x_start, y_track_2, x_end, y_track_2),
        linewidth=2,
        linecolor="gray",
        screen_coordinates=False,
    )
    sim.Animate3dLine(
        x0=x_start, y0=y_track_2, z0=0,
        x1=x_end, y1=y_track_2, z1=0,
        color="gray",
    )

    # Add Sleepers (Horizontal lines across tracks)
    sleeper_spacing = 3  # Distance between sleepers
    for x in range(x_start, x_end, sleeper_spacing):
        sim.AnimateLine(
            spec=(x, y_track_1, x, y_track_2),
            linewidth=1,
            linecolor="brown",
            screen_coordinates=False,
        )
        sim.Animate3dLine(
            x0=x, y0=y_track_1, z0=0,
            x1=x, y1=y_track_2, z1=0,
            color="brown",
        )

class Train(sim.Component):
    def setup(self, num_trolleys=20, start_x=100, start_y=21, speed=0.1):
        ...
        # Initialize current_x for cumulative position
        current_x = start_x

        # Create trolleys with containers
        for i in range(num_trolleys):
            # Randomly select container length and color
            length = sim.Pdf((20, 40), (50, 50))()
            color = sim.Pdf(colors, 1)()
            # Calculate length in model units
            container_length_units = length * 12.2 / 40  # Same conversion as in Container class

            # Adjust trolley x position with minimal spacing
            trolley_x = current_x

            # Create trolley (flat car) with matching length
            trolley = AnimateTrolley(
                x=lambda t, tx=trolley_x: tx + self.speed * t,
                y=self.start_y,
                color="blue",
                length=container_length_units,
            )
            # Create container using the Container class
            container = Container(
                length=length,
                color=color,
                edge_color="",
                equipment=None,
                x_now=lambda t, tx=trolley_x: tx + self.speed * t,  # Update position over time
                y_now=self.start_y,
                z_now=3,  # Height above ground level
            )
            self.trolleys.append((trolley, container))

            # Update current_x for next trolley
            current_x -= (container_length_units + 0.1)  # Adjust for container length and minimal spacing


    def process(self):
        # No need to update positions; they update dynamically
        while True:
            self.hold(0.1)  # Small time step for smooth animation

class TrainRTG(sim.Component):
    def setup(self, x_position, y_position, trolley_speed=1, spreader_speed=1):
        self.x_position = x_position
        self.y_position = y_position
        self.trolley_pos = 0
        self.trolley_direction = 1
        self.spreader_height = 15
        self.spreader_direction = -1
        self.trolley_speed = trolley_speed
        self.spreader_speed = spreader_speed

        # Create the RTG animation
        self.ao = AnimateRTG(
            x0=lambda t: self.x_position,
            y0=lambda t: self.y_position,
            z_angle=0,
            trolley_pos=lambda t: self.trolley_pos,
            spreader_height=lambda t: self.spreader_height,
            spreader_length=12.2,
        )

    def process(self):
        while True:
            # Update trolley position
            self.trolley_pos += self.trolley_speed * self.trolley_direction
            if self.trolley_pos > PORTALWIDTH / 2 or self.trolley_pos < -PORTALWIDTH / 2:
                self.trolley_direction *= -1  # Reverse direction at edges

            # Update spreader height
            self.spreader_height += self.spreader_speed * self.spreader_direction
            if self.spreader_height > 15 or self.spreader_height < 5:
                self.spreader_direction *= -1  # Reverse direction at limits

            self.hold(1)  # Short time step for smooth animation


class AnimateTrolley(sim.Animate3dBox):
    def __init__(self, x, y, color="gray", length=12.2):
        # Convert color to fixed RGB tuple
        fixed_color = env.colorspec_to_tuple(color)

        super().__init__(
            x_len=length,
            y_len=CONTAINER_WIDTH * 1.2,
            z_len=1,
            x=x,
            y=y,
            z=1,  # Set z position lower
            color=fixed_color,
            shaded=True,
        )

    def move(self, dx):
        # Update x position dynamically
        if callable(self.x):
            current_x = self.x(env.now())
            self.x = lambda t: current_x + dx * (t - env.now())
        else:
            self.x += dx
...
Source(position=Point(400, 30), iat=sim.Uniform(60, 180), type_=sim.CumPdf(("in", 50, "out", 100)), v=v_external_trucks)
Source(position=Point(200, 220), iat=sim.Uniform(60, 180), type_=sim.CumPdf(("in", 100, "out", 100)), v=v_internal_trucks)
Source(position=Point(600, 225), iat=sim.Uniform(60, 180), type_=sim.CumPdf(("in", 0, "out", 100)), v=v_internal_trucks)
...
colors = ["crimson", "forestgreen", "royalblue", "firebrick", "seagreen", "steelblue", "indianred", "mediumseagreen", "navy"]
...
# Call add_tracks to add tracks after defining blocks
add_tracks()

# Add train
Train(num_trolleys=100, start_x=100, start_y=55, speed=0.1)
# Add RTGs at both ends of the tracks
# Add Train RTGs
# Add Train RTGs with explicit speed parameters
train_rtg_start = TrainRTG(x_position=20, y_position=60, trolley_speed=1, spreader_speed=1)
train_rtg_end = TrainRTG(x_position=720, y_position=60, trolley_speed=1, spreader_speed=1)



sim.Animate3dGrid(x_range=range(0, 801, 100), y_range=range(0, 301, 100))  # Extended x range

env.view.x_eye = 1500
env.view.y_eye = -1500
env.view.z_eye = 600
env.view.x_center = 380
env.view.y_center = 160
env.view.z_center = 0
env.view.field_of_view_y = 8.0
