        ...
        if self.type == 'reefer':
            if self.modal == 'train':
                dwell_time = random.normalvariate(
                    container_types["reefer"]["dwell_time"]["train"]["mean"],
                    container_types["reefer"]["dwell_time"]["train"]["std"]
                ) * 24 * 60
            else:  # truck
                dwell_time = random.normalvariate(
                    container_types["reefer"]["dwell_time"]["truck"]["mean"],
                    container_types["reefer"]["dwell_time"]["truck"]["std"]
                ) * 24 * 60
        else:  # dry
            if self.modal == 'train':
                dwell_time = random.normalvariate(
                    container_types["dry"]["dwell_time"]["train"]["mean"],
                    container_types["dry"]["dwell_time"]["train"]["std"]
                ) * 24 * 60
            else:  # truck
                dwell_time = random.normalvariate(
                    container_types["dry"]["dwell_time"]["truck"]["mean"],
                    container_types["dry"]["dwell_time"]["truck"]["std"]
                ) * 24 * 60
...
class PortSimulation:
    ...
    def process_container(self, container):
        ...
        if yard[container.modal].level < yard[container.modal].capacity:
            ...
            # Handle departure based on modal type
            if container.modal == "truck":
                if container in container_list:
                    yield self.env.process(self.handle_truck_departure(container))
            else:  # train containers will be handled by train departure process
                # Mark container as ready for train
                container.ready_time = self.env.now
    ...
    def handle_train_departure(self):
        ...
        current_hour = int((self.env.now / 60) % 24)  # Change back to % 24
                ...
                for _ in range(regular_take):
                    for c in container_list:
                        if not hasattr(c, 'departure_time'):
                            ...
                            departure_hour = int((self.env.now / 60) % 24)  # Change back to % 24
...
def main():
    ...
    st.sidebar.markdown("### Container Dwell Times")
    st.sidebar.markdown("#### Dry Containers")
    dry_truck_mean = st.sidebar.number_input("Dry Truck Mean (days)", 
                                            value=CONTAINER_TYPES_DEFAULT["dry"]["dwell_time"]["truck"]["mean"], 
                                            step=0.1)
    dry_truck_std = st.sidebar.number_input("Dry Truck Std (days)", 
                                           value=CONTAINER_TYPES_DEFAULT["dry"]["dwell_time"]["truck"]["std"], 
                                           step=0.1)
    dry_train_mean = st.sidebar.number_input("Dry Train Mean (days)", 
                                            value=CONTAINER_TYPES_DEFAULT["dry"]["dwell_time"]["train"]["mean"], 
                                            step=0.1)
    dry_train_std = st.sidebar.number_input("Dry Train Std (days)", 
                                           value=CONTAINER_TYPES_DEFAULT["dry"]["dwell_time"]["train"]["std"], 
                                           step=0.1)
    
    st.sidebar.markdown("#### Reefer Containers")
    reefer_truck_mean = st.sidebar.number_input("Reefer Truck Mean (days)", 
                                               value=CONTAINER_TYPES_DEFAULT["reefer"]["dwell_time"]["truck"]["mean"], 
                                               step=0.1)
    reefer_truck_std = st.sidebar.number_input("Reefer Truck Std (days)", 
                                              value=CONTAINER_TYPES_DEFAULT["reefer"]["dwell_time"]["truck"]["std"], 
                                              step=0.1)
    reefer_train_mean = st.sidebar.number_input("Reefer Train Mean (days)", 
                                               value=CONTAINER_TYPES_DEFAULT["reefer"]["dwell_time"]["train"]["mean"], 
                                               step=0.1)
    reefer_train_std = st.sidebar.number_input("Reefer Train Std (days)", 
                                              value=CONTAINER_TYPES_DEFAULT["reefer"]["dwell_time"]["train"]["std"], 
                                              step=0.1)

    # Update container_types with the new values
    container_types = {
        "dry": {
        ...
        "reefer": {
            ...
            "dwell_time": {
                "truck": {
                    "mean": reefer_truck_mean,
                    "std": reefer_truck_std
                },
                "train": {
                    "mean": reefer_train_mean,
                    "std": reefer_train_std
                }
            }
        }
    }
